...
//NCORE--;
#ifdef THREADPO
	//2023f BS::thread_pool_light thread_pool(NCORE-1);
	//20230408 BS::thread_pool thread_pool(NCORE-1);
	//printf("info thread_pool(%d) created\n", NCORE-1);
	printf("info thread_pool(%d) created\n", NCORE); //20230408
	fflush(stdout);
#endif
...
//	char *BackSlashPtr;
	...
	//printf("Eychessu 2.892w (physical NCORE=%d, popcnt=%d) by Edward Yu\n", NCORE, POPCNT_CPU);
	printf("Eychessu 2023g (logical NCORE=%d, popcnt=%d) by Edward Yu\n", NCORE, POPCNT_CPU);
	fflush(stdout);
#ifdef THREADPO
	printf("thread_pool(%d) created\n", NCORE-1);
	fflush(stdout);
#endif
...
//bef exchess						ext_IMaxTime =  max(8, (UcciComm.nTime/25) + UcciComm.nIncrement );  //*3/64=/21 /2 // /25 /28 /25 /22        
      ...
      // 2892v vs XQMS  //if (UcciComm.nTime < 1000) 
                        ...
                        if (p_movenum <= 20) 
                        		ext_IMaxTime = min(UcciComm.nTime / 2, (UcciComm.nTime/(45-p_movenum)) + UcciComm.nIncrement) ;
                        else
                        	ext_IMaxTime = (UcciComm.nTime/(p_movenum)) + UcciComm.nIncrement; 
